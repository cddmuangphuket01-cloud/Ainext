...
(() => {
  ...
  const OPENROUTER_GPT_MODEL='openrouter/free';
  ...
  const getModel=p=>p==='openai'?OPENROUTER_GPT_MODEL:(p==='gemini'||p==='google'?'gemini-3.5-flash':(window.PROVIDERS?.[p]?.model||DEFAULT_MODEL));
  ...
  async function backendChat(provider,message,prior,model){
    const effectiveModel=(provider==='gemini'||provider==='google')?'gemini-3.5-flash':(provider==='openai'?'openrouter/free':model);
    const r=await fetch(FUNCTION_URL,{method:'POST',headers:{'Content-Type':'application/json','apikey':SUPABASE_KEY,'Authorization':`Bearer ${SUPABASE_KEY}`,'x-ainext-client-id':S.clientId},body:JSON.stringify({provider,model:effectiveModel,prompt:message,messages:Array.isArray(prior)?prior:[]})});
