...
(() => {
  ...
  async function saveTurn(userMessage, assistantMessage) {
    try {
      if (!dbConversationId) {
        const { data, error } = await db.from('ai_conversations').insert({ client_id: CLIENT_ID, title: String(userMessage).slice(0, 80) || 'การสนทนาใหม่', provider: 'google', model: 'gemini-2.0-flash' }).select('id').single();
      ...
      if (error) throw error;
      await db.from('ai_usage').insert({ client_id: CLIENT_ID, conversation_id: dbConversationId, provider: 'google', model: 'gemini-2.0-flash', request_type: 'chat', status: 'success' });
    } catch (e) { console.error('Ainext DB save error:', e); }
  }
  ...
  if (typeof originalCallAI === 'function') {
    window.callAI = async function(providerKey, userMessage, priorMessages) {
      ...
      const response = await fetch(FUNCTION_URL, { method: 'POST', headers: { 'Content-Type': 'application/json', 'apikey': SUPABASE_KEY }, body: JSON.stringify({ messages: priorMessages || [], prompt: userMessage }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || `AI HTTP ${response.status}`);
      await saveTurn(userMessage, data.text || '(ไม่มีคำตอบจากโมเดล)');
      return data.text || '(ไม่มีคำตอบจากโมเดล)';
    };
  }
  const originalNewChat = window.newChat;
  if (typeof originalNewChat === 'function') {
    window.newChat = function() { localStorage.removeItem(CONV_KEY); dbConversationId = null; return originalNewChat.apply(this, arguments); };
  }
